...
if [[ ${PV} != *9999* ]]; then
	...
	SRC_URI="https://github.com/Molunerfinn/PicGo/archive/refs/tags/v${PV}.tar.gz -> ${PF}.tar.gz"
	S="${WORKDIR}/PicGo-${PVR}"
else
	inherit git-r3 desktop xdg-utils
	EGIT_REPO_URI="https://github.com/Molunerfinn/PicGo.git"
fi
...
src_prepare() {
	default
	yarn install --network-timeout 1000000 || die "Yarn failed to install dependencies"
}
...
#}
src_compile() {
	yarn electron:build --dir || die "Yarn failed to build PicGO"
}
...
src_install() {
	...
	dodir /usr/bin/
	cp -R ${S}/dist_electron/linux-unpacked/* ${D}/usr/share/picgo/ || die "Install failed!"
	ln -s ${D}/usr/share/picgo/picgo ${D}/usr/bin/picgo
	make_desktop_entry "picgo %U" PicGO  Utility
	doicon  -s 256 -t  "${S}/build/icons/256x256.png"
}
pkg_postinst() {
